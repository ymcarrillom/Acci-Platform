  ...
  async (req, res, next) => {
    try {
      ...
      function drawCell(doc, x, y, w, h, text, opts = {}) {
        ...
        doc.text(text, x + pad, y + (h - fontSize) / 2, {
          width: w - pad * 2,
          align,
          lineBreak: false,
        });
      ...
      for (const [code, label, color] of legendItems) {
        drawCell(doc, lx, legendY, 20, 16, code, {
          bg: color,
          fg: '#ffffff',
          bold: true,
          align: 'center',
      ...
      function drawAttHeader(y, datesToShow) {
        ...
        for (const d of datesToShow) {
          ...
          drawCell(doc, x, y, dateColW, headerH, label, {
            bg: headerBg,
            fg: headerFg,
            bold: true,
            align: 'center',
        ...
        drawCell(doc, x, y, rateColW, headerH, '% Asis.', {
          bg: headerBg,
          fg: headerFg,
          bold: true,
          align: 'center',
      ...
      for (let chunkIdx = 0; chunkIdx < dateChunks.length; chunkIdx++) {
        ...
        if (dateChunks.length > 1) {
          doc
            ...
            .fill('#64748b')
            .text(
              `Sesiones ${chunkIdx * datesPerPage + 1} a ${chunkIdx * datesPerPage + datesToShow.length} de ${dates.length}`,
              tableLeft
            );
          doc.moveDown(0.3);
        }
        ...
        for (let i = 0; i < students.length; i++) {
                ...
                status === 'PRESENT'
                  ...
                  : status === 'ABSENT'
                    ...
                    : status === 'LATE'
                      ? '#d97706'
                      : '#2563eb';
              drawCell(doc, x, y, dateColW, rowH, label, {
                bg,
                fg: color,
                bold: true,
                align: 'center',
          ...
          drawCell(doc, x, y, rateColW, rowH, `${rate}%`, {
            bg,
            fg: rateFg,
            bold: true,
            align: 'center',
      ...
      doc
        ...
        .font('Helvetica-Bold')
        .fill('#1e293b')
        .text('Resumen General de Asistencia', tableLeft);
      doc.moveDown(0.5);
      ...
      function drawSummaryHeader(atY) {
        ...
        for (let c = 0; c < sCols.length; c++) {
          drawCell(doc, sx, atY, sCols[c], headerH, sLabels[c], {
            bg: headerBg,
            fg: headerFg,
            bold: true,
      ...
      for (let i = 0; i < students.length; i++) {
        ...
        sx += sCols[1];
        drawCell(doc, sx, sy, sCols[2], rowH, `${s.present}`, {
          bg,
          fg: '#059669',
          bold: true,
          align: 'center',
        ...
        sx += sCols[2];
        drawCell(doc, sx, sy, sCols[3], rowH, `${s.absent}`, {
          bg,
          fg: '#dc2626',
          bold: true,
          align: 'center',
        ...
        sx += sCols[3];
        drawCell(doc, sx, sy, sCols[4], rowH, `${s.late}`, {
          bg,
          fg: '#d97706',
          bold: true,
          align: 'center',
        ...
        sx += sCols[4];
        drawCell(doc, sx, sy, sCols[5], rowH, `${s.excused}`, {
          bg,
          fg: '#2563eb',
          bold: true,
          align: 'center',
        ...
        const rateFg = rate >= 80 ? '#059669' : rate >= 60 ? '#d97706' : '#dc2626';
        drawCell(doc, sx, sy, sCols[6], rowH, `${rate}%`, {
          bg,
          fg: rateFg,
          bold: true,
          align: 'center',
