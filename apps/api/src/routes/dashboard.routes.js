  ...
  try {
    ...
    if (role === "ADMIN") {
      ...
      const recentActivities = await prisma.activity.findMany({
        ...
        select: {
          id: true, title: true, type: true, courseId: true, createdAt: true,
      ...
      return res.json({
        role,
        fullName,
        metrics: { totalUsers, activeUsers, sessionsActive, totalCourses, pendingGrading, totalActivities },
        recentCourses,
        recentActivities,
      });
    }
...
router.get("/pending-feedback", authRequired, requireRole("TEACHER", "ADMIN"), async (req, res, next) => {
  try {
    const teacherCourseIds = (
      await prisma.course.findMany({
        where: req.user.role === "ADMIN" ? { isActive: true } : { teacherId: req.user.sub, isActive: true },
        select: { id: true },
      })
    ).map((c) => c.id);
...
router.get("/group-tracking/:courseId", authRequired, requireRole("TEACHER", "ADMIN"), async (req, res, next) => {
  try {
    ...
    const studentSummaries = students.map((s) => ({
      ...
      risk: (attendanceByStudent[s.id].pct !== null && attendanceByStudent[s.id].pct < 60) ||
            (gradesByStudent[s.id].avg !== null && gradesByStudent[s.id].avg < 60),
    }));

    // Group averages
    const allGraded = submissions.filter((s) => s.grade !== null);
    const groupGradeAvg = allGraded.length > 0
      ? Math.round(allGraded.reduce((sum, s) => sum + s.grade, 0) / allGraded.length)
      : null;

    const allAttPcts = Object.values(attendanceByStudent).filter((a) => a.pct !== null).map((a) => a.pct);
    const groupAttendanceAvg = allAttPcts.length > 0
