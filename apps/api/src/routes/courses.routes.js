...
router.get("/:id/grades/export", authRequired, requireRole("TEACHER", "ADMIN"), async (req, res, next) => {
  try {
    const course = await prisma.course.findUnique({
      where: { id: req.params.id },
    ...
    res.setHeader("Content-Disposition", `attachment; filename="calificaciones-${course.code}.pdf"`);
    doc.pipe(res);
    ...
    function drawCell(doc, x, y, w, h, text, opts = {}) {
      ...
      doc.font(bold ? "Helvetica-Bold" : "Helvetica").fontSize(8).fill(fg);
      const pad = align === "left" ? 5 : 3;
      doc.text(text, x + pad, y + (h - 8) / 2, { width: w - pad * 2, align, lineBreak: false });
    }

    // ── Page header ──
    doc.fontSize(16).font("Helvetica-Bold").fill("#1e293b").text("Reporte de Calificaciones", { align: "center" });
    doc.moveDown(0.2);
    ...
    function drawGradesHeader(y, actsToShow, tableW) {
      ...
      for (const act of actsToShow) {
        ...
        drawCell(doc, x, y, actColW, headerH, label, { bg: headerBg, fg: headerFg, bold: true, align: "center" });
        x += actColW;
      }
      drawCell(doc, x, y, avgColW, headerH, "Promedio", { bg: headerBg, fg: headerFg, bold: true, align: "center" });
    }
    ...
    for (let chunkIdx = 0; chunkIdx < actChunks.length; chunkIdx++) {
      ...
      if (actChunks.length > 1) {
        doc.fontSize(8).font("Helvetica").fill("#64748b")
          .text(`Actividades ${startIdx + 1} a ${startIdx + actsToShow.length} de ${activities.length}`, tableLeft);
        doc.moveDown(0.3);
      }
      ...
      for (let i = 0; i < studentRows.length; i++) {
        ...
        drawCell(doc, x, y, avgColW, rowH, row.avg, { bg, fg: avgFg, align: "center", bold: true });
        y += rowH;
      }
    }
    ...
    // ── Summary page ──
    doc.addPage();
    doc.fontSize(14).font("Helvetica-Bold").fill("#1e293b").text("Resumen de Calificaciones", tableLeft);
    doc.moveDown(0.5);
    ...
    function drawSummaryHeader(y) {
      drawCell(doc, tableLeft, y, sCols[0], headerH, "Estudiante", { bg: headerBg, fg: headerFg, bold: true });
      drawCell(doc, tableLeft + sCols[0], y, sCols[1], headerH, "Promedio", { bg: headerBg, fg: headerFg, bold: true, align: "center" });
      drawCell(doc, tableLeft + sCols[0] + sCols[1], y, sCols[2], headerH, "Calificadas / Total", { bg: headerBg, fg: headerFg, bold: true, align: "center" });
    }
    ...
    for (let i = 0; i < studentRows.length; i++) {
      ...
      const avgFg = row.avg === "-" ? "#94a3b8" : Number(row.avg) >= 6.0 ? "#059669" : "#dc2626";
      drawCell(doc, sx, sy, sCols[1], rowH, row.avg, { bg, fg: avgFg, align: "center", bold: true });
      sx += sCols[1];

      const graded = row.grades.filter((g) => g !== "-" && g !== "Pend.").length;
      drawCell(doc, sx, sy, sCols[2], rowH, `${graded} / ${activities.length}`, { bg, align: "center" });
      sy += rowH;
    }
...
router.get("/:id/students/export", authRequired, requireRole("TEACHER", "ADMIN"), async (req, res, next) => {
  try {
    const course = await prisma.course.findUnique({
      where: { id: req.params.id },
      include: { teacher: { select: { fullName: true } } },
    ...
    function drawCell(doc, x, y, w, h, text, opts = {}) {
      ...
      if (bg) { doc.save().rect(x, y, w, h).fill(bg).restore(); }
      doc.save().rect(x, y, w, h).lineWidth(border).stroke(borderC).restore();
      doc.font(bold ? "Helvetica-Bold" : "Helvetica").fontSize(9).fill(fg);
      const pad = align === "left" ? 6 : 3;
      doc.text(text, x + pad, y + (h - 9) / 2, { width: w - pad * 2, align, lineBreak: false });
    }

    // ── Page header ──
    doc.fontSize(16).font("Helvetica-Bold").fill("#1e293b").text("Lista de Estudiantes", { align: "center" });
    doc.moveDown(0.2);
    doc.fontSize(9).font("Helvetica").fill("#64748b").text("Academia de Crecimiento Cristiano Integral (ACCI)", { align: "center" });
    doc.moveDown(0.6);
    ...
    function drawTableHeader(y) {
      ...
      for (let i = 0; i < headers.length; i++) {
        drawCell(doc, x, y, colW[i], headerH, headers[i], {
          bg: headerBg, fg: headerFg, bold: true, align: i === 0 ? "center" : "left",
        });
        x += colW[i];
      }
    }
    ...
    for (let i = 0; i < students.length; i++) {
      ...
      x += colW[2];
      drawCell(doc, x, ty, colW[3], rowH,
        new Date(s.enrolledAt).toLocaleDateString("es-CO", { day: "2-digit", month: "short", year: "numeric" }),
        { bg, align: "center" }
      );
      ty += rowH;
    }
    ...
    if (students.length === 0) {
      drawCell(doc, tableLeft, ty, tableW, rowH, "No hay estudiantes inscritos", { align: "center" });
    }
...
router.get("/:id/students", authRequired, requireRole("ADMIN", "TEACHER"), async (req, res, next) => {
  try {
...
router.delete("/:id/enroll/:studentId", authRequired, requireRole("ADMIN"), async (req, res, next) => {
  try {
