  ...
  async (req, res, next) => {
    try {
      ...
      if (activity.maxAttempts > 0 && existingCount >= activity.maxAttempts) {
        return res
          .status(409)
          .json({ message: `Has alcanzado el máximo de ${activity.maxAttempts} intento(s)` });
      }
    ...
    try {
      ...
      if (activity.type === 'QUIZ' && activity.timeLimit) {
        ...
        if (!startToken) {
          return res
            .status(400)
            .json({ message: 'Token de inicio requerido para quizzes con tiempo límite' });
        }
        try {
          ...
          if (elapsedSeconds > limitSeconds + 30) {
            // 30 seconds grace period
            return res
              .status(400)
              .json({ message: `Tiempo límite superado (${activity.timeLimit} min)` });
          }
        } catch {
      ...
      if (activity.type === 'QUIZ') {
        ...
        if (!parsed.success) {
          return res
            .status(400)
            .json({ message: 'Datos inválidos', errors: parsed.error.flatten() });
        }
        ...
        try {
          submission = await prisma.$transaction(async (tx) => {
            ...
            if (activity.maxAttempts > 0 && existingCount >= activity.maxAttempts) {
              const err = new Error(
                `Has alcanzado el máximo de ${activity.maxAttempts} intento(s)`
              );
              err.status = 409;
              throw err;
            }
...
router.get(
  '/:activityId/submissions/:submissionId',
  authRequired,
  verifyCourseAccess,
  async (req, res, next) => {
    try {
...
router.patch(
  '/:activityId/submissions/:submissionId/grade',
  authRequired,
  requireRole('TEACHER', 'ADMIN'),
  verifyCourseAccess,
  async (req, res, next) => {
...
router.delete(
  '/:activityId/submissions/:submissionId/reset',
  authRequired,
  requireRole('TEACHER', 'ADMIN'),
  verifyCourseAccess,
  async (req, res, next) => {
