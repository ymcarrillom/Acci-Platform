  ...
  if (data?.role !== "ADMIN") {
    return (
      <div className="rounded-3xl border border-white/10 bg-slate-950/55 backdrop-blur-xl p-8 shadow-2xl text-center">
        ...
        <p className="text-slate-200/80 mt-2">Solo administradores pueden crear usuarios.</p>
        <Link href="/dashboard/usuarios" className="inline-block mt-4 rounded-xl bg-white/10 border border-white/10 px-4 py-2 text-sm font-bold text-white hover:bg-white/15">
          Volver a usuarios
        </Link>
      </div>
    );
